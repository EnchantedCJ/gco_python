...
cdef extern from "GCoptimization.h":
    ...
    cdef cppclass GCoptimizationGeneralGraph:
        ...
        void setDataCost(float *) except +
        void setSmoothCost(float *) except +
        void setLabelCost(float *) except +
        void setNeighbors(int, int) except +
        void setNeighbors(int, int, float) except +
        void expansion(int n_iterations) except +
        void swap(int n_iterations) except +
        int whatLabel(int node) except +
...
def cut_from_graph(np.ndarray[np.int32_t, ndim=2, mode='c'] edges,
        np.ndarray[np.float32_t, ndim=1, mode='c'] weight,
        np.ndarray[np.float32_t, ndim=2, mode='c'] unary_cost,
        np.ndarray[np.float32_t, ndim=2, mode='c'] pairwise_cost,
        np.ndarray[np.float32_t, ndim=1, mode='c'] label_cost, 
        n_iter=5, algorithm='expansion'):
    """
    Apply multi-label graphcuts to arbitrary graph given by `edges`. Set label cost separately.
    Data type is np.float32 (float in ctype).

    Parameters
    ----------
    edges: ndarray, int32, shape(n_edges, 2)
        Rows correspond to edges in graph, given as vertex indices.
    weight: ndarray, float32, shape=(n_edges, 1)
        Weight of edges used in smooth cost.
    unary_cost: ndarray, float32, shape=(n_vertices, n_labels)
        Unary potentials
    pairwise_cost: ndarray, float32, shape=(n_labels, n_labels)
        Pairwise potentials for label compatibility
    label_cost: ndarray, float32, shape=(n_labels, )
        Cost per label. Penalizes the number of distinct labels in the solution
    n_iter: int, (default=5)
        Number of iterations
    algorithm: string, `expansion` or `swap`, default=expansion
    ...
    if pairwise_cost.shape[1] != pairwise_cost.shape[0]:
        raise ValueError("pairwise_cost must be a square matrix.")
    if label_cost.shape[0] != pairwise_cost.shape[0]:
        raise ValueError("label_cost length must be n_labels.")
    cdef int n_vertices = unary_cost.shape[0]
    cdef int n_labels = pairwise_cost.shape[0]

    cdef GCoptimizationGeneralGraph* gc = new GCoptimizationGeneralGraph(n_vertices, n_labels)
    for e, w in zip(edges, weight):
        gc.setNeighbors(e[0], e[1], w)
    gc.setDataCost(<float*>unary_cost.data)
    gc.setSmoothCost(<float*>pairwise_cost.data)
    gc.setLabelCost(<float*>label_cost.data)
    if algorithm == 'swap':
        gc.swap(n_iter)
    elif algorithm == 'expansion':
        gc.expansion(n_iter)
